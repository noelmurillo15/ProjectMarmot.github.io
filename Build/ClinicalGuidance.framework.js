...
function _InitializeFileLoader(callbackObjectName, callbackMethodName) {
 ...
 if (!fileuploader) {
  ...
  fileuploader.onchange = function(e) {
   ...
   unityInstance.SendMessage("PersistentGameManager", FileCallbackMethodName, URL.createObjectURL(files[0]));
  };
 }
}

function _InitializeJsLib() {
...
function _OpenPdfFileNewTab(base64Data) {
 let pdfWindow = window.open("");
 if (pdfWindow) {
  pdfWindow.document.write("<iframe width='100%' height='100%' src='data:application/pdf;base64, " + encodeURI(UTF8ToString(base64Data)) + "'></iframe>");
 } else {
  var result = confirm("Preview PDF?");
  if (result) {
   ...
   try {
    ...
    alert("Pop-up Blocker is enabled! Cannot preview PDF unless this website is added as an exception");
   }
  }
 }
}

var _PDFRenderer_loadedDocuments = [];

var _PDFRenderer_loadedDocumentLoadingTasks = [];

function _PDFJS_CloseDocument(documentHandle) {
...
var _PDFRenderer_loadedPages = [];

function _PDFJS_ClosePage(pageHandle) {
 var page = _PDFRenderer_loadedPages[pageHandle - 1];
 _PDFRenderer_loadedPages[pageHandle - 1] = undefined;
 delete page;
}

var _PDFRenderer_loadedCanvas = [];

function _PDFJS_DestroyCanvas(canvasHandle) {
 var canvas = _PDFRenderer_loadedCanvas[canvasHandle - 1];
 delete canvas;
 _PDFRenderer_loadedCanvas[canvasHandle - 1] = undefined;
}

function _PDFJS_GetPageCount(documentHandle) {
 var pdfDocument = _PDFRenderer_loadedDocuments[documentHandle - 1];
 return pdfDocument.numPages;
}

function _PDFJS_GetPageHeight(pageHandle, scale) {
 return _PDFRenderer_loadedPages[pageHandle - 1].getViewport({
  scale: scale
 }).height;
}

function _PDFJS_GetPageWidth(pageHandle, scale) {
 return _PDFRenderer_loadedPages[pageHandle - 1].getViewport({
  scale: scale
 }).width;
}

function _PDFJS_InitLibrary() {
...
function _PDFJS_LoadDocumentFromBytes(promiseHandle, base64) {
...
function _PDFJS_LoadDocumentFromURL(promiseHandle, url) {
 var promiseHandleString = Pointer_stringify(promiseHandle);
...
function _PDFJS_LoadPage(promiseHandle, documentHandle, pageIndex) {
 var promiseHandleString = Pointer_stringify(promiseHandle);
 var pdfDocument = _PDFRenderer_loadedDocuments[documentHandle - 1];
...
function _PDFJS_RenderCanvasIntoTexture(canvasHandle, textureHandle) {
 var canvas = _PDFRenderer_loadedCanvas[canvasHandle - 1];
 GLctx.pixelStorei(GLctx.UNPACK_FLIP_Y_WEBGL, true);
...
var _PDFRenderer_workers = [];

function _PDFJS_RenderPageIntoCanvas(promiseHandle, pageHandle, scale, width, height) {
 var promiseHandleString = Pointer_stringify(promiseHandle);
 var page = _PDFRenderer_loadedPages[pageHandle - 1];
...
function _PDFJS_TryTerminateRenderWorker(promiseHandle) {
...
function _RequestUserFile(extensions) {
 ...
 var fileuploader = document.getElementById("fileuploader");
 if (fileuploader === null) InitializeFileLoader(FileCallbackObjectName, FileCallbackMethodName);
...
var asmLibraryArg = {
 "CancelFullscreen": _CancelFullscreen,
 "InitializeFileLoader": _InitializeFileLoader,
 "InitializeJsLib": _InitializeJsLib,
 ...
 "OpenPdfFileNewTab": _OpenPdfFileNewTab,
 "PDFJS_CloseDocument": _PDFJS_CloseDocument,
 "PDFJS_ClosePage": _PDFJS_ClosePage,
 "PDFJS_DestroyCanvas": _PDFJS_DestroyCanvas,
 "PDFJS_GetPageCount": _PDFJS_GetPageCount,
 "PDFJS_GetPageHeight": _PDFJS_GetPageHeight,
 "PDFJS_GetPageWidth": _PDFJS_GetPageWidth,
 "PDFJS_InitLibrary": _PDFJS_InitLibrary,
 "PDFJS_LoadDocumentFromBytes": _PDFJS_LoadDocumentFromBytes,
 "PDFJS_LoadDocumentFromURL": _PDFJS_LoadDocumentFromURL,
 "PDFJS_LoadPage": _PDFJS_LoadPage,
 "PDFJS_RenderCanvasIntoTexture": _PDFJS_RenderCanvasIntoTexture,
 "PDFJS_RenderPageIntoCanvas": _PDFJS_RenderPageIntoCanvas,
 "PDFJS_TryTerminateRenderWorker": _PDFJS_TryTerminateRenderWorker,
 "QuitCleanup": _QuitCleanup,
